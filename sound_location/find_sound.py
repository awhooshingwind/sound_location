...
s_syncKC = abs(kpeaks_filtered[1] - cpeaks_filtered[1])
print(s_syncKC, s_syncKJ)

#%% Selecting Claps from filtered signal peaks
clap_timestamps = np.array([7, 17.5, 40, 80, 131]) # approx times, in seconds, around peaks of interest
...
for label, value in claps.items():
    ...
    diffs_KC.append(abs(value[0] - value[2] + s_syncKC))
...
c_KJ = 15 # ft
c_KC = 11.5 # ft
b_KJ = np.sqrt(abs(c_KJ**2 - a_KJ**2)) # already ^2
b_KC = np.sqrt(abs(c_KC**2 - a_KC**2)) # already ^2

#%% Generate points
theta = np.radians(88.1)
lim = 35
N = 1000
...
ax.plot(kevin_phone[s_syncKJ:], label='k_phone')
ax.plot(curtis_phone[s_syncKC:], label='c_phone')
